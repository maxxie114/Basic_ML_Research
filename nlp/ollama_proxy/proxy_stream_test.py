...
def log_request_info():
    ...
    # 🔥 FIXED: Avoid consuming the request body early
    try:
        json_body = request.get_json()  # This does not consume the stream like get_data()
        app.logger.debug('Request JSON Body: %s', json_body)
    except Exception as e:
        app.logger.debug('Could not parse request body as JSON: %s', str(e))
...
def log_response_info(response):
    ...
    # 🔥 FIXED: Do NOT log response body for streaming responses
    if response.direct_passthrough:  # This means it's a streaming response
        app.logger.debug('Streaming response - body not logged')
    else:
        app.logger.debug('Response Body: %s', response.get_data(as_text=True))  # Safe for non-streaming
